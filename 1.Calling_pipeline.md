...
In this markdown I explain the trials I have done to test different versions of GATK with few individals.
...
To work around the very long times it would take for the analysis to be run on all individuals for the whole genome, I will work on a "parallelization" pipeline in order to call variants for each chromosome simultaneously on all the individuals. See 1.Parallel_Calling.md for details on that.
