...
(5) Under-represented, excessively missing variants
(6) Over/Under covered regions (<5% and >95% of depth distribution)
(7) Genes?
...
# 1610638 - 1610633 = 5
```
## (1,2,3,4) Applying General Filters

The script applying filters 1 through 4, which are independent of species and sequencing technology, and can therefore be applied to the whole dataset can be found at 2.Variant_Filtering-executables/General_Filter_1-2-3-4.sh
...
Before calculating coverage and depth distributions I want to first divide my dataset by species and sequencing technology, as they represent key factors in determining which areas of the genome are more or less sequenced (species specific structural variation might cause the different sequencing machines to work differently; species specific duplications; etc.).

This is how the dasaset will be divided:

Lynx rufus - three groups : c_lr_fl_0005 from Murphy; c_lr_nm_0006 from Janecka; c_lr_zz_0001 with MACROGEN

Lynx canadiensis - two groups : c_lc_zz_0001 with MACROGEN; c_lc_zz_0003 from Murphy

Lynx pardinus - two groups : c_lp_do_0153, c_lp_do_0173, c_lp_do_0443, c_lp_sm_0138, c_lp_sm_0140, c_lp_sm_0185, c_lp_sm_0186, c_lp_sm_0298, c_lp_sm_0359, h_lp_do_0007 from Proyecto Genoma; c_lp_sm_0221 from CANDILES

Lynx lynx - one group : c_ll_cr_0212, c_ll_ki_0090, c_ll_vl_0112, c_ll_ya_0146 with MACROGEN

I'll create BAMlist files (a file with a list of BAMs) for each group

```
# Lynx lynx - one group : cr_0212, ki_0090, vl_0112, ya_0146 with MACROGEN
ls $LUSTRE/test/CatRef_bams/c_ll_*_cat_ref_sorted_rg_rmdup_sorted_indelrealigner.bam > $LUSTRE/test/CatRef_bams/MACROGEN.bamlist

```

As the datasets are very big and we only need a representative subset of the data, I will subsample the BAM files. To do that I will generate a BED file with X random positions of Y length and change its format so ANGSD can read it. The depth calculations will be done considering only this random subset.
...
# redirect to awk to change format to ANGSD region file (useful only if using ANGSD):
bedtools random -l 100000 -n 100 -g $LUSTRE/test/Felis_catus_Ref/Felis_catus.Felis_catus_9.0.dna.toplevel.genome | \
awk '{printf ("%s:%s-%s\n", $1, $2, $3)}' | sort > $LUSTRE/test/FilterTrials/Felis_catus.100x100kbp.genome.rf
...
Without ANGSD on CESGA's ft2, I will calculate depth at each position using samtools depth. Depth at all positions will be calculated (-a) within the regions randomly selected before (-b). I will try it on the MACROGEN samples first.

```
# Samtools depth:
samtools depth -a -b $LUSTRE/test/FilterTrials/Felis_catus.100x100kbp.masked.genome.bed \
-f $LUSTRE/test/CatRef_bams/MACROGEN.bamlist \
> $LUSTRE/test/FilterTrials/MACROGEN.100x100kbp.masked.depth
```

This table of per-position depth values will be analyzed in R.
