...
# After the first PGENOMA mapping, this script will be used to go through the same
# steps (mapping, adding read groups and merging, marking duplicates and realigning)
# with the remaining samples of the GENOMA project.

# The remaining fastq files were transfered to the genomics-b.ebd.csic.es server from
...
for i in ${PGenoma2ARRAY[@]}
  do
    ...
    echo -e " ********************** \n\n - Mapping ${i} - \n\n **********************"
    bwa mem $REF $PGenoma2PATH/${i}_1.fastq.gz $PGenoma2PATH/${i}_2.fastq.gz \
...
SampleARRAY=($(for i in ${PGenoma2ARRAY[@]}; do echo ${BARCODEID["${i}"]}; done | sort -u))
...
for i in ${SampleARRAY[@]}
  do
    ...
    -nt $THREADS -R $REF -I $OUT/${i}_cat_ref_sorted_rg_rmdup_sorted.bam \
